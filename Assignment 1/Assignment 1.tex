...
% \\
% &+ \text{Other terms for fair share of distances and match gaps} 
\end{align*}
...
% \[    
    % 0 <= L_{i,j,n} <= 1
%     L_{i,j,n} = \sum_{d} x_{i,j,d}) (count_{j,d} - n + 1)
% = 1 if j played their nth match on day d at stadium i, else 0
% \]
% L_{j,j,n} >= \sum_{i} x_{j,i,d} (count_{j,d} - n + 1)
% L_{i,j,n} <= 1 and L_{i,j,n} >= 0
% dist_{i,j}L_{i,k,n-1}L_{j,k,n} + dist_{i,j}L_{i,j,n-1}L_{j,j,n} + dist_{i,j}L_{i,i,n-1}L_{j,i,n}

% Need to linearize the above formulation for distance
% L_{i,j,n,d} = 1 if x_{i,j,d}) (count_{j,d} - n + 1) = 1 where i \neq j
% L_{j,j,n} = 1, if \sum_{i} x_{j,i,d} (count_{j,d} - n + 1) = 1
% 0 <= L_{i,j,n} <= 1

%  Final formulation
%  \sum_{d} x_{i,j,d}) (count_{j,d} - n + 1) <= 1 +  M(1 - L_{i,j,n})
%  \sum_{d} x_{i,j,d}) (count_{j,d} - n + 1) >= 1 - M(1 - L_{i,j,n})

% L_{j,j,n}
% \sum_{i} \sum_{d} x_{j,i,d} (count_{j,d} - n + 1) <= 1 + M(1 - L_{j,j,n})
% \sum_{i} \sum_{d} x_{j,i,d} (count_{j,d} - n + 1) >= 1 - M(1 - L_{j,j,n})
\end{document}
