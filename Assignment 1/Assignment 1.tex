...
\begin{itemize}
    ...
    \item $\text{p\_away}_{i,d}$: Binary variable, 1 if team $i$ has played atleast one away match by day $d$, else 0.
    \item $\text{p\_home}_{i,d}$: Binary variable, 1 if team $i$ has played atleast one home match by day $d$, else 0.
...
\begin{enumerate}
    ...
    \begin{align*}
        \text{count\_away}_{i,d} &\leq \text{p\_away}_{i,d} \cdot M \quad \forall i \in T, \forall d \in D\\
        \text{count\_away}_{i,d} &\geq \text{p\_away}_{i,d} \quad \forall i \in T, \forall d \in D\\
        \text{count\_home}_{i,d} &\leq \text{p\_home}_{i,d} \cdot M \quad \forall i \in T, \forall d \in D\\
        \text{count\_home}_{i,d} &\geq \text{p\_home}_{i,d} \quad \forall i \in T, \forall d \in D\\
        \text{count\_home}_{i,d} - \text{count\_away}_{i,d} &\leq \text{MaxConsecHome} - \text{p\_away}_{i,d} \quad \forall i \in T , \forall d \in D \\
        \text{count\_away}_{i,d} - \text{count\_home}_{i,d} &\leq \text{MaxConsecAway} - \text{p\_home}_{i,d} \quad \forall i \in T , \forall d \in D
    \end{align*}
    For the above constraints,
    \begin{itemize}
        \item The first two constraints ensure that $\text{count\_away}_{i,d} = 0$, if $\text{p\_away}_{i,d} = 0$ and $\text{count\_away}_{i,d} \geq 1$, if $\text{p\_away}_{i,d} = 1$
        \item The next two constraints ensure that $\text{count\_home}_{i,d} = 0$, if $\text{p\_home}_{i,d} = 0$ and $\text{count\_home}_{i,d} \geq 1$, if $\text{p\_home}_{i,d} = 1$
        \item The last two constraints ensures that we don't have more than MaxConsecAway consecutive away matches and not more than MaxConsecHome consecutive home matches for any team.
        \item The last two constraints also bound the difference between the number of home and away matches for any given day of the round robin, so that the home and away matches are fairly distributed across time for all teams.
    \end{itemize}
